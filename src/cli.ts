...
program
  ...
  .option("--static-classifier <mode>", "Structural Semantic Classifier integration. Only mode currently supported: \"report-only\" — classify every file with the deterministic AST-based classifier (src/runtime/legend/structural-classifier.ts) and surface aggregates in the INGEST report. Does NOT change LLM routing, does NOT skip files, does NOT alter ensemble behavior. Pure observation. Use to inspect the distribution of barrels / schemas / adapters / etc. across a perimeter before considering future enabled-mode pruning.")
  .option("--json", "Output results in JSON format.")
