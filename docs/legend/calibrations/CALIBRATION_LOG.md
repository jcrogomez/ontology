...
- [REGEN_ORACLE_REFINE_2026-06-17](REGEN_ORACLE_REFINE_2026-06-17.md) — **exploratory** (build + measurement, companion to the above): wired the diagnosed unused levers into the real machinery — **(1) oracle-into-generation** (behaviour-fixture acceptance criteria → compile-back system prompt), **(2) verify-refine loop** (`--refine N`, deterministic gates as the verifier, draft-side diagnostics fed back), **(3) static self-containment lint → refine** (`draft-lint.ts`: undefined-reference calls + async-where-signature-is-sync) — plus a robustness fix (`withRegenDraftGuard` contains a dropped-helper draft's deferred throw that otherwise crashed the run) and a trustworthy hand-written `lock.ts` behaviour oracle. **Measured on glue `node_0013`: structural jaccard 0.538 → 1.000 (over-export drift eliminated); each lever fixed its TARGETED defect — verified by reading the drafts (refine → sync export surface; lint → `acquireLock` made synchronous + the undefined `registerExitHook` defined) — but the behaviour gate did NOT pass.** Layered finding: intent fully consumed (every fix the levers targeted landed in a subsequent draft); the residual is a **local-7B capacity limit on holding a 309-line side-effectful contract coherently in one generation** (no single high-variance draw keeps sync + self-containment + all exports + full lock-detection + ownership-release + the right filename simultaneously). Next lever was decomposition (below). Pure `node_0110` unregressed (jaccard 1.000). $0, local 7B. **Addendum — lever (4) decomposition built + measured: regenerate in slices (scaffold→per-export) with an ownership-aware assembler (the 7B regenerates the whole module per slice, so dedup by ownership is required), composing with refine. Result: behaviour 0/5 → 3/5 on `node_0013` (acquire / stale-reclaim / idempotent-release PASS; lint fixed the async override). Then lever (3) richer extraction landed (capture determining const literals into slice grounding + state the observable lock path in the oracle), which CLOSED the path gap (regen now writes `.ontology/.lock`, not `.lock.json`), plus best-round tracking (the 7B regresses round-to-round) and a gate fix (untested ≠ writeable when a fixture is present). **Final stable state: 3/5** (acquire / stale-reclaim / idempotent-release pass). The last 2 failures (cross-host, ownership-release) are now pure LOGIC bugs the 7B produces inconsistently — passing a formatted string instead of the error detail object; release unlinking without an ownership check — both described in the oracle. Consumption neck closed; residual is a local-7B capability + variance limit on two logic details. Path to 5/5 = consensus-over-many-draws or the frontier comparison, not another consumption lever. The levers took a hard glue node 0→3/5, each fixing its targeted defect (verified by reading the code).** **Uncontaminated OPEN confirmation: `qwen3-coder:480b-cloud` (open, free on Ollama cloud; GLM-5.2 is paid-only) on the SAME pipeline+gate reaches 4/5 COLD (no levers, one shot) and 5/5 with refine — behaviour PASS on the glue node, where the local 7B needs all four levers for 3/5. Confirms the neck was local-model capacity, not consumption/machinery. (One fixture correction landed: the cross-host case had compared the error's cosmetic `.name`; corrected to test the contract `detail.kind`+foreign-lock-preserved — identity still 5/5, the 7B's substantive cross-host miss still fails.) Plus a latent test-helper fix: `smallestInstalledModel` now excludes cloud models (size-0 in /api/tags) so the live dispatcher test doesn't route to a gated cloud model after `ollama signin`.**
