...
- **§3.1 metric circularity check (Arm A0).** Launched 2026-05-24T18:36Z: `qwen2.5-coder:7b` + safety-net **without** `--ast-grounding`, identical perimeter to Arm A (cloned from `.ontology.self-ingest-epsilon-3a-arm-a-result/`). Isolates the marginal contribution of grounding from the safety-net contribution and the metric-circularity confound. Output: `SELF_INGEST_EPSILON_3A_2026-05-19_ARM_A0_CONTROL.md` + `.ontology.self-ingest-epsilon-3a-arm-a0.json`. Expected wall-clock ~1h 33min (matched Arm A's swap-floor measurement). Workspace post-run: `.ontology.self-ingest-epsilon-3a-arm-a0-result/`. Pre-existing scratch `.ontology/` (6 nodes, unrelated test fixtures) stashed at `.ontology.scratch-pre-arm-a0-<ts>/`.
