...
- 🟢 **Bilateral round-trip — unit side of `G ⊣ F` measured 2026-06-12 ($0).** First-ever measurement of intent→code→intent over the live 228-node graph (3 arms via cold-subagent frontier replay, [`ROUNDTRIP_BILATERAL_2026-06-12`](legend/calibrations/ROUNDTRIP_BILATERAL_2026-06-12_REPORT.md) triplet). **Unit side round-trips:** Arm A median M1 (contract) 0.80, `provides` recovery 1.0, M2 beats null 92–98% all arms. **Kernel-of-equivalence map = 19/48 nodes structurally regenerable (T2)**, 15 on the local $0 stack — the dated input for ficha refinement. **Binding constraint identified with clean cross-arm attribution: extraction/ficha quality, not compiler F** — M1 falls A→B→C on thin 3B-extracted originals but *rises* on S7 frontier-extracted originals (peak C 0.60, the only place rules survive). Confirms + sharpens the "extraction completeness" lever below. Counit baseline refreshed (structural Jaccard 0.667; contract 0.638, below the 0.726 May anchor). §3.10 stays **T2** (first unit-triangle tolerance, not a theorem). **Follow-ons:** ✅ **`onto regenerate <node>` shipped 2026-06-13** — the governed lever (preview-by-default; `--write` gated on a structure-preserving verdict + behaviour non-regression; overwriting `src/` is an artefact write governed by *verification*, not the proposal fence). First live run vindicated preview-by-default: a single-draw 7B regen of a kernel node dropped an `export` and introduced an off-by-one, and the gate refused the write (`tests/regenerate-cli.test.ts`, 5/5). ✅ **`onto probe <node>` shipped 2026-06-13** — the behavioural safety net for `regenerate --write`. An LLM proposes characterization cases from source + contract; each is run against the real source and **only self-validated cases persist** (honest by construction). Closes the structural-verdict blind spot: demonstrated live that a structurally-identical `nodeCount+1` off-by-one scores `epsilon_equivalent` (jaccard 1.0, would be written) yet **fails the behaviour fixture and blocks the write**. Probe quality tracks model capability — the local 7B anchored on stale intent and self-validation refused all cases (wrote nothing), the same extraction bottleneck the round-trip measured. `tests/probe-cli.test.ts` (7/7). ✅ **Kernel behavioural lift 2026-06-13** — auto-`probe` over the 19-node kernel cohort lifted **11 nodes to behavioural-T2** (119 self-validated cases, $0 frontier; [`KERNEL_BEHAVIOURAL_LIFT_2026-06-13`](legend/calibrations/KERNEL_BEHAVIOURAL_LIFT_2026-06-13.md)). 4 not characterizable (I/O / CLI / types-only / walker), 4 excluded by category. Fixtures registered + green in the smoke guard (47/47). `regenerate --behavior-check --write` now has a real net for those 11. ✅ **Multi-draw consensus 2026-06-13** — `regenerate --draws N [--consensus K]`: compile N independent drafts (distinct cache-bypass tokens), cluster the write-acceptable ones by declaration set, write the majority class only at the consensus floor K. Defangs single-draw *variance* (the behaviour gate defangs systematic ficha error — the two are complementary). Live on node_0017 the 7B gave three different drafts (two off-by-one-with-export, one correct-but-no-export) → consensus 0/3, write refused — the exact danger single-draw `--write` would hit. `tests/regenerate-cli.test.ts` 9/9. Still open: frontier `--write`; mutation-scoring of probe discriminating power; future M1 against a frontier-extracted *reference* ficha to dissolve the reference-frame confound.
