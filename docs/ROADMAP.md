...
- 🟡 **Behaviour-axis checker — v0 spec landed.** [`legend/BEHAVIOUR_AXIS_CHECKER_SPEC.md`](legend/BEHAVIOUR_AXIS_CHECKER_SPEC.md). Cartography matrix currently fills only `structural` (+ `cost`); `contract` / `behaviour` / `intent` are explicit no-data. Behaviour is next-highest-value because it is **orthogonal to AST grounding** and so immune to §3.1 circularity. v0 is a smoke runtime-equivalence check over a curated ~20-node subset; ~4–6 h estimated implementation; can ship before or after Arm C-cloud (backfill against archived workspaces is $0).
