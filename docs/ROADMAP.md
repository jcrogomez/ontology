...
- 🟢 **Intent→code regeneration loop shipped 2026-06-12 → 14 ($0).** The governed bidirectional loop, built + measured end-to-end. Dated records: [`ROUNDTRIP_BILATERAL_2026-06-12`](legend/calibrations/ROUNDTRIP_BILATERAL_2026-06-12_REPORT.md), [`KERNEL_BEHAVIOURAL_LIFT_2026-06-13`](legend/calibrations/KERNEL_BEHAVIOURAL_LIFT_2026-06-13.md), [`LENS_LAWS_2026-06-13`](legend/calibrations/LENS_LAWS_2026-06-13_REPORT.md). PRs #148–#151.
  ...
  - 🟡 **Ficha-cleanup pass STARTED 2026-06-14** — the product lever the whole arc points at. `onto ficha audit` (read-only, measure-before-construct) quantified the deficiency: **138/221 code nodes under-declare their exports — 430 missing export declarations** (the recall-bound thinness, worst in the large multi-export modules), + 57 prose/noise rules. `onto ficha cleanup <node> [--apply]` ships the deterministic high-confidence fix (complete the contract with the AST export surface — no LLM). `tests/ficha-cli.test.ts` 4/4. **Not yet applied to the live graph** (the user's call). Remaining cleanup (LLM/judgment, the Walker loop): apply contract-completion across the worklist; prompt refinement; rule denoising; re-extract thin prompts via frontier.
