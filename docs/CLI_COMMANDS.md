...
### `probe <nodeId>` *(2026-06-13 — behavioural safety net for regenerate)*

- **Purpose:** generate a **self-validated behavioural fixture** for a
  node — the safety net that gives `onto regenerate --write` teeth. An
  LLM proposes characterization cases from the source + contract; each
  case is then run against the **real source** (source-vs-source through
  the behaviour-checker) and **only the cases that cleanly match are
  persisted** to `tests/behavior-fixtures/<nodeId>.fixture.ts`. The
  fixture is therefore a set of TRUE statements about how the code
  behaves today.
- **Why it matters:** the structural verdict alone cannot catch a
  behavioural bug. A regeneration that keeps the export signature but
  flips an off-by-one scores `epsilon_equivalent` (jaccard 1.0) and
  would be **written** — the behaviour fixture catches it (`fail`) and
  blocks the write. Demonstrated live: a structurally-identical
  `nodeCount + 1` regen passed structure but failed behaviour.
- **Honest by construction:** a case that throws, times out, is
  non-deterministic, or asserts something false about the source is
  **dropped** — only validated cases persist. A weak local model that
  writes intent-anchored (wrong) asserts simply yields zero kept cases
  and **writes nothing** (observed: qwen2.5-coder:7b anchored on the
  stale "increment" intent and self-validation refused all four). Probe
  quality tracks model capability — the same extraction-quality
  bottleneck the bilateral round-trip measured.
- **Rule-targeting (the executable enforcement layer, 2026-06-14):** when the
  node has **behavioural** rules (per `onto rules` classification — "returns X
  when Y", "throws on Z"), probe feeds them as numbered verification targets
  and asks for one `rule:<N> — …` case each. Self-validation then turns each
  into an **enforced** invariant (a passing case the regenerate behaviour-gate
  locks) or surfaces it as **`⚠ code may violate this rule`** when the case is
  dropped (the source failed its own declared rule — enforcement by
  construction). Demonstrated live: all 5 behavioural rules of
  `reps-aggregator.ts` (node_0225) generated passing cases → all 5 enforced.
  The `ruleCoverage` JSON field reports per-rule `enforced` /
  `violated_or_unassertable` / `uncovered`.
- **Governance:** a hand-written fixture (no `GENERATED by` header) is
  protected — `--force` to replace it. Generated fixtures are always
  replaceable.
- **Flags:** `--provider`, `--model`, `--ollama-host`, `--force`,
  `--fixtures-dir <path>`, `--max-tokens`, `--json`.
- **Runs under tsx** (`npm run dev -- probe ...`) — the behaviour-checker
  imports modules dynamically and needs the TS-aware host.
- **Output (JSON):** `{ ok, nodeId, sourceFile?, fixturePath?, generatedCases?, keptCases?, dropped?: [{name, outcome}], written, failure? }`.

### `regenerate <nodeId>` *(2026-06-13 — the governed lever)*

- **Purpose:** make the kernel-of-equivalence map
  ([`ROUNDTRIP_BILATERAL_2026-06-12`](legend/calibrations/ROUNDTRIP_BILATERAL_2026-06-12_REPORT.md))
  actionable per node. Regenerate a node's **code shadow** from its
  intent (the forward functor F), verify the candidate against the
  source on disk, and — **only with `--write`, and only when the
  regeneration is structure-preserving** — overwrite the real source
  file. This is the operation that turns "code is a regenerable shadow
  of intent" from a measurement into a daily move.
- **Governance (not the proposal fence — verification + force-gate):**
  overwriting a real `src/` file is an artefact write, not a graph
  mutation, so the proposal system is the wrong fence. The fence here is
  *verification*:
  - **Default (preview):** compile-back to `.ontology/verify/<id>.<ext>`,
    diff vs the shadow, report the verdict. **Touches no source.**
  - **`--write`:** applies **only if** the verdict ∈ {`epsilon_equivalent`,
    `divergent_loc`} (structure preserved) **and**, when `--behavior-check`
    finds a fixture, behaviour does not regress. Otherwise it **refuses**
    and exits non-zero — it will not clobber working source with a
    divergent regeneration. After a write, the operator reviews the diff
    and re-anchors drift (`onto drift --update`) as separate explicit acts.
- **Why preview-by-default:** single-draw local-model regeneration is not
  reliably safe to write even for kernel nodes (a first live run dropped
  an `export` and introduced an off-by-one; the gate caught both). Use a
  stronger model, multi-draw consensus, or human review before `--write`.
- **Multi-draw consensus (`--draws N [--consensus K]`):** a single draw is
  unsafe in two independent ways — random **variance** (one draw drops an
  `export` or a token) and systematic **error** (a stale ficha mis-specifies
  the behaviour). The two gates address them separately: consensus across N
  draws defangs variance (an outlier draft is outvoted), `--behavior-check`
  defangs the systematic case. With `--draws` the command compiles N
  independent drafts (distinct cache-bypass token each), clusters the
  write-acceptable ones by declaration set, and writes the majority class
  only when it reaches the floor K (default strict majority). Live on
  node_0017 the 7B produced three *different* drafts — two off-by-one with
...
the Project Legend phases are detailed in [`PROJECT_LEGEND.md`](design/inverse/PROJECT_LEGEND.md).
