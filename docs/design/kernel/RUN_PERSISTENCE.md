...
5. Should runs dispatched by a [scanner](../proposals/WAKEUP_SCANNERS.md) carry a back-reference to the bundle that will group their resulting proposals? Same shape as #2 (optional update event `run_bundle_attached`), same trade-off (append-only purity vs convenience of in-record back-reference). Lean: yes, via an event rather than mutating the run body.
