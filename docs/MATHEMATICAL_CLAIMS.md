...
**Framing correction 2026-07-21 (no tier change).** The Axiom 5 gluing entry
was worded as a "sheaf on the equal-signature *subcategory / sub-site*". That
conflates two things: a cover `{Uᵢ}` carries no signatures until a section is
placed on it, so *signature-agreement is the sheaf's matching (equalizer)
condition, not a restriction of the coverage*. The precise, now-proved statement:
the context presheaf `F` (partial maps `U ⇀ Sig`) is a **sheaf on the full
standard site** `(P_fin(K), ⊆)` with the joint-surjection coverage — which is a
genuine **Grothendieck** pretopology (identity / stability-under-pullback (= ∩) /
transitivity all hold, three set-identities) — and the amalgamation is general
(arbitrary covers, any `n`, any keys; the old 2-piece sweep is its finite
shadow). The two policies are then the sheaves of **two value-presheaves over
that one site**: `identify-if-equal` = the signature sheaf `F_Sig` (provenance
forgotten; unknown signature conservatively treated as an obstruction), and the
default = the provenance-tagged `F_Node` sheaf (SSoT). Everything pinned before
(existence / restriction round-trip / uniqueness / boundary) stands; only the
categorical *language* is corrected and generalised. Full development +
code-verified predicates: [`design/laws/GLUING_SITE_THEOREM.md`](design/laws/GLUING_SITE_THEOREM.md).
...
- **Tier:** T1 (strictly implemented) for the **presheaf-restriction law**; T2 (operationally implemented) characterised down to a **separated presheaf, explicitly *not* a sheaf**, for the **default** gluing operation (negative law pinned 2026-06-01); **T1 (strictly implemented) for the opt-in `identify-if-equal` gluing as the sheaf gluing of the signature presheaf on the standard site** (equal-signature-on-overlaps is the sheaf's *matching* condition, NOT a sub-coverage — 2026-07-21 framing correction above; the site is Grothendieck, [`design/laws/GLUING_SITE_THEOREM.md`](design/laws/GLUING_SITE_THEOREM.md)) — the gluing axiom is pinned as a characterising law over an explicit cover with restriction maps (promoted 2026-06-09, O2 + Path-to-T1 gate #2 — see below).
...
- Axiom 5 (gluing, `identify-if-equal` opt-in): **the signature-sheaf gluing on the standard site** (`(P_fin(K),⊆)` with the joint-surjection coverage — a genuine Grothendieck pretopology; equal-signature-on-overlaps is the *matching* condition, not a sub-coverage — framing corrected + generalised 2026-07-21, [`design/laws/GLUING_SITE_THEOREM.md`](design/laws/GLUING_SITE_THEOREM.md)). Gluing axiom pinned as a characterising law over an explicit cover with restriction maps (`restrictSection`, `presheaf-sheaf-laws.test.ts` Part 3: existence + restriction round-trip + well-definedness/reconstruction (section-uniqueness follows) + exhaustive sweep over all 49 ordered two-piece families (28 distinct) + boundary; promoted 2026-06-09, Path-to-T1 gate #2) and proved generally for arbitrary covers. Matching currently decided by the syntactic discriminator; the resolved-type refinement is a fidelity axis, not a law gap.
