...
- **Code:** `src/runtime/graph/poset.ts`; enforcement at `src/commands/node/link.ts`, `src/commands/proposal/propose-link.ts`, and `src/commands/validate.ts` (note: `src/core/edges/create-edge.ts` itself does **not** check poset direction — the check lives at the command entry points).
...
- **Rigor improvement:** drop the rewrite-rule framing from `PROPOSAL_SYSTEM.md` §1 and replace it with "typed candidate mutation with full provenance" — what the system actually delivers. The rewrite-rule framing *is* accurate elsewhere — see [`PROMPT_GENERATORS.md`](design/forward/PROMPT_GENERATORS.md), where `@expand: gen_xxx` substitution in generator bodies is actual rewriting and lifts Axiom 4 to T2 in that domain (see §Axiom 4 above) — but proposals themselves are typed mutations, not rewrites.
