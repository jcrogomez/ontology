...
- **Path to T1.** Close all three: (1) replace the syntactic proxy with a **resolved-type signature** (a whole-program `TypeChecker` pass) so "identical signature" means genuine interface-identity, eliminating false non-matches — **extractor built + feasibility-pinned 2026-06-09** (`src/runtime/static/typescript-resolved.ts`, `tests/typescript-resolved.test.ts`: resolves inferred returns / inferred consts the syntactic tier cannot, tier-tagged via `RESOLVED_SIGNATURE_PREFIX` so it never string-equals a syntactic signature); **still to do: wire it into the ingest path** so produced nodes carry resolved signatures; (2) pin the **sheaf gluing axiom as a stated law over an explicit cover** (existence + uniqueness of the glued section for a compatible family), the same lift the §3.8 universal-property test makes — not just identify/conflict behaviour cases; (3) ship a real consumer — **done (O3/O4):** `onto workflow run --as-proposal` produces contracted nodes the policy reconciles. (1)-wiring + (2) are the remaining substantive gates.
